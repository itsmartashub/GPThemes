...
import { initStorageManager } from './storageManager.js'

async function init() {
	console.log(await browser.storage.sync.get('gptheme'))
	initUI()
	const themeManager = await initThemeManager()
	const storageManager = await initStorageManager()
	initSettingsManager(themeManager, storageManager)
	console.log(await browser.storage.sync.get('gptheme'))
}
...
// File: ui.js
import { icon_paint, icon_sun, icon_moon, icon_moon_full, icon_settings } from '../app/components/icons.js'
...
import { fontHtmlCode, handleFontsListeners } from '../app/mainFonts.js'
import { assetsHtmlCode, handleAssetsListeners } from '../app/mainAssets.js'
import { hexToHSL } from '../utils/hexToHSL.js'

const DEFAULT_COLOR_LIGHT = '#7e3e47'
const DEFAULT_COLOR_DARK = '#ca93fb'

export function initSettingsManager(themeManager, storageManager) {
	renderSettings()
	handleColorInput()
}
...
export async function setAccentToStorage(storageColorProperty, accentValue) {
	try {
...
// Utility functions (can be moved to a separate file if desired)
